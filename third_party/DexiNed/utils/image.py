...
''' DexiNed utils with modification'''
...
def save_image_batch_to_disk(tensor, output_dir, file_names, img_shape=None, arg=None, is_inchannel=False, mask=None, single_channel=False):
    ...
    if arg.mode == 'train':
        ...
        for i_shape, file_name in zip(image_shape, file_names):
            ...
            if single_channel: # single scale of DexiNed output, which is last scale(fused)
                if arg.thr1 == -1:
                    fuse = (255*(1-tmp[-1])).astype(np.uint8)
                else:
                    fuse = (1-tmp[-1])*255
                    fuse[fuse<arg.thr1]*=0.3 # re-scale intensities
                    fuse = fuse.astype(np.uint8)
